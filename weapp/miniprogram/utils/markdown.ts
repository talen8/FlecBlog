...
/**
 * 预处理 Markdown 内容（兼容旧接口）
 * 将自定义块语法转换为标准 Markdown
 * @param markdown - 原始 Markdown 内容
...
export function preprocessMarkdown(markdown: string): string {
  ...
  for (const block of blocks) {
    if (block.type === 'markdown') {
      ...
      const emoji = emojiMap[block.noteType] || '💡';
      const titleLine = block.title ? `**${emoji} ${block.title}**\n\n` : `**${emoji} 提示**\n\n`;
      result.push(`> ${titleLine}${block.content}\n`);
      ...
      const linkText = block.title || block.link;
      const desc = block.description ? ` - ${block.description}` : '';
      result.push(`[${linkText}](${block.link})${desc}`);
    } else if (block.type === 'video') {
      if (block.platform === 'bilibili') {
        result.push(`> 🎬 [观看 B站视频](https://www.bilibili.com/video/${block.videoId})`);
      } else if (block.platform === 'youtube') {
        result.push(`> 🎬 [观看 YouTube视频](https://www.youtube.com/watch?v=${block.videoId})`);
      } else {
        result.push(`> 🎬 [观看视频](${block.platform})`);
      }
    } else if (block.type === 'audio') {
      result.push(`> 🎵 **${block.title}** - [点击播放](${block.audioUrl})`);
      ...
      result.push(`> 🎵 **在线音乐** - ${serverNames[block.server] || block.server}`);
      ...
      for (const row of block.rows) {
        for (const img of row) {
          ...
          if (imgMatch) {
            images.push(`![${imgMatch[1] || '图片'}](${imgMatch[2] || img})`);
          } else {
            images.push(`![图片](${img})`);
          }
        }
      }
      result.push(images.join('\n\n'));
    }
  }
