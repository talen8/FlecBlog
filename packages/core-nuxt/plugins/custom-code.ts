...
export default defineNuxtPlugin({
  ...
  setup() {
    ...
    useHead(
      computed(() => {
        const customHead = buildHeadPayload(basicConfig.value.custom_head || '');
        const fontLink = buildFontLink(basicConfig.value.font || '');
    ...
    const injectBodyCode = () => {
      const bodyCode = basicConfig.value.custom_body || '';
