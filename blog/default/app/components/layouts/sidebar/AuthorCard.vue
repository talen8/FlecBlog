...
const articlesTotal = computed(() => siteStats.value.total_articles);
const categoriesTotal = computed(() => siteStats.value.total_categories);
const tagsTotal = computed(() => siteStats.value.total_tags);

const contacts = computed(() => {
  const socialList = parseJSON<Array<{ name: string; url: string; icon: string }>>(
    themeConfig.value.sidebar_social,
    []
  );
