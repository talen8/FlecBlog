...
// 计算属性
const currentTrack = computed<AudioTrack | null>(() => audioList.value[currentIndex.value] || null);
...
const playTrack = async (index: number) => {
  ...
  lyrics.value = track?.lrc ? await fetchLyrics(track.lrc) : [];
...
onMounted(async () => {
  await load();
  const first = audioList.value[0];
  if (first?.lrc) lyrics.value = await fetchLyrics(first.lrc);
});
</script>
