...
const { submit, queryByTicket } = useFeedback();
...
const handleFileUpload = (event: Event, targetKey: keyof typeof fileArrayMap) => {
  ...
  for (const file of files) {
    const validationError = validate(file, '反馈投诉');
    if (validationError) {
...
const uploadFiles = async (files: File[]): Promise<string[]> => {
  ...
  const uploadPromises = files.map(file => upload(file, '反馈投诉'));
...
const handleQueryTicket = async () => {
  ...
  try {
    queryResult.value = await queryByTicket(queryTicketNo.value.trim());
    success('查询成功');
  } catch (error: unknown) {
    const err = error as Error;
