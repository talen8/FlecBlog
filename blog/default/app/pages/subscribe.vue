...
const handleSubscribe = async () => {
  ...
  try {
    ...
    errorToast(err.data?.message || err.message || '订阅失败');
  } finally {
...
const handleUnsubscribe = async (token: string) => {
  ...
  try {
    ...
    unsubscribeMessage.value = err.data?.message || err.message || '退订失败';
  } finally {
...
<template>
  <div id="subscribe-page">
    ...
    <UiBaseDialog
      ...
      :title="themeConfig.wechat_qrcode ? '关注公众号' : '公众号订阅'"
      confirm-text=""
    >
      <div class="wechat-dialog-content">
        <template v-if="themeConfig.wechat_qrcode">
          <p class="wechat-desc">
            扫描二维码关注公众号<br />
            获取最新文章推送
          </p>
          <div class="qrcode-wrapper">
            <NuxtImg
              :src="themeConfig.wechat_qrcode"
              :alt="themeConfig.wechat_name || '公众号二维码'"
              class="qrcode-image"
              loading="lazy"
            />
          </div>
          <p v-if="themeConfig.wechat_name" class="wechat-name">
            {{ themeConfig.wechat_name }}
          </p>
        </template>
        <template v-else>
          <p class="wechat-desc">
