...
const info = computed(() => {
  ...
  const personalityCode = theme.about_personality || 'INFJ-A';
  const personality = getPersonalityInfo(personalityCode);

  return {
    author: basicConfig.value.author || '',
    describe: theme.about_describe || '',
    describeTips: theme.about_describe_tips || '',
    photo: theme.author_photo || '',
    exhibitionImg: theme.about_exhibition || '',
    ...
    motto: {
      main: parseJSON<Array<{ value: string }>>(theme.about_motto_main, []).map(m => m.value),
      sub: theme.about_motto_sub || '',
    },
    socialize: parseJSON<Array<{ name: string; url: string }>>(theme.about_socialize, []),
    creation: parseJSON<Array<{ name: string; url: string }>>(theme.about_creation, []),
    versions: parseJSON<Array<{ name: string; version: string }>>(theme.about_versions, []),
    union: parseJSON<Array<{ name: string; url: string }>>(theme.about_unions, []),
    story: theme.about_story || '',
  };
});

const runningDays = computed(() => {
  const established = themeConfig.value.established || '2024-01-01';
    ...
    <div id="two">
      ...
      <div class="information">
        ...
        <div class="about-layout Statistics">
          <span>{{ siteStats.total_articles }}篇文章</span>
          <span>{{ siteStats.total_categories }}个分类</span>
          <span>{{ siteStats.total_tags }}个标签</span>
