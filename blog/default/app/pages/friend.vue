...
const { data: allGroups } = await useFriends();

// 正常友链分组（is_invalid = false）
const friendGroups = computed(() =>
  (allGroups.value ?? [])
    .map(group => ({ ...group, friends: group.friends.filter(f => !f.is_invalid) }))
...
// 空状态判断
const isEmpty = computed(() => (allGroups.value ?? []).every(group => group.friends.length === 0));
</script>
